...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter { //(1)
    ...
    private String delegateRefreshToken(User user) {
        ...
        redisTemplate.opsForValue()
                .set(user.getEmail(), refreshToken, jwtTokenizer.getRefreshTokenExpirationMinutes(), TimeUnit.MINUTES);
...
- (4)
: Override 을 통해 Custom 하는 메서드
: 로그인 인증에 성공했을 경우에 Authentication(인증서)에 저장된 회원 정보(User)를 통해 JWT(String)를 만들고 이를 response header 에 추가하는 메서드

- (4-1)
: Authentication(인증서)의 Principal 에 User 객체(정보)가 저장되어 있다.
- (4-2)
: JWT(String)을 만드는 부분
- (4-3)
...
: (5), (6)의 차이는 User 의 정보인 Claims 의 유무의 차이이다.
 */
